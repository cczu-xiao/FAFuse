...
def deit_small_patch16_224(pretrained=False, **kwargs):
    model = DeiT(
        patch_size=16, embed_dim=768, depth=10, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),**kwargs)   # img_size=384
    ...
    pe = F.interpolate(pe, size=(22,22), mode='bilinear', align_corners=True)       # patchµÄ´óÐ¡
    pe = pe.flatten(2)
    pe = pe.transpose(-1, -2)
